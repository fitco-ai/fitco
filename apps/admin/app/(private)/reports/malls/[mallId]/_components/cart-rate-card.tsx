...
import { useCartAnalyticsByMallId } from '@/queries/dashboard';
import {
  Card,
  CardContent,
  CardHeader,
...
export function CartRateCard({ mallId }: { mallId: number }) {
  const { selectedShopNo } = useMallReportContext();
  const { data, isLoading } = useCartAnalyticsByMallId(mallId, selectedShopNo);
