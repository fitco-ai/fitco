...
import { useRefundRate } from '@/queries/salesvolume';
...
export function ProductRefundRateCard({ mallId }: { mallId: number }) {
  const [period, setPeriod] = useState<ComparePeriod>('1m');
  const { selectedShopNo } = useMallReportContext();
  const { data: refundReport, isLoading } = useRefundRate(
    mallId,
    selectedShopNo,
    period
  );
  ...
  const chartData = useMemo(() => {
    ...
    return [
      { period: '설치 전', rate: refundReport.before.refundRate },
      { period: '설치 후', rate: refundReport.after.refundRate },
    ];
  }, [refundReport]);
        ...
        <div className="h-72">
          {isLoading ? (
            <div className="flex h-full w-full items-center justify-center">
              <LoadingSpinner message="반품률 데이터를 불러오는 중..." />
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ left: 8, right: 8 }}>
                ...
                />
                <Tooltip
                  formatter={(v: number) => [`${v}%`, '반품률']}
                  labelFormatter={(d) => d}
                />
                <Bar dataKey="rate" fill="#6366F1" maxBarSize={20} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
